...
// @brief non-blocking version of getPressure
// @param baro_pressure: pointer to float to put the barometer pressure read
// @return READ_SUCCESS on completion and STILL_READING when still reading
int Adafruit_MPL3115A2::getPressure(float* baro_pressure) {
  uint32_t pressure;

  static bool read_flag = 1;
  if(read_flag) {
      read_flag = read8(MPL3115A2_CTRL_REG1) & MPL3115A2_CTRL_REG1_OST;
 ...
 */
float Adafruit_MPL3115A2::getPressureBlocking() {
  uint32_t pressure;
...
// @brief non-blocking version of getAltitude
// @param altitude: pointer to float to put the barometer pressure read
// @return READ_SUCCESS on completion and STILL_READING when still reading
int Adafruit_MPL3115A2::getAltitude(float* altitude) {
  int32_t alt;
 ...
 *  @return altitude reading as a floating-point value
 */
float Adafruit_MPL3115A2::getAltitudeBlocking() {
  int32_t alt;
...
// @param altitude: pointer to float to put the barometer pressure read
// @return READ_SUCCESS on completion and STILL_READING when still reading
int Adafruit_MPL3115A2::getTemperature(float* temp) {
  int16_t t;
  ...
  static bool sta_flag = 1;
  if(sta_flag) {
      if(!(sta & MPL3115A2_REGISTER_STATUS_TDR)) {
          sta = read8(MPL3115A2_REGISTER_STATUS);
          sta_flag = 0;
      } else {
          return STILL_READING;
 ...
 */
float Adafruit_MPL3115A2::getTemperatureBlocking() {
  int16_t t;
