...
#define STILL_READING 0 // for non-blocking functions
...
public:
  ...
  int getPressure(float* baro_pressure);
  float getPressureBlocking(void);
  int getAltitude(float* altitude);
  float getAltitudeBlocking(void);
